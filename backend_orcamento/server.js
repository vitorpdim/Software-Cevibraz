...
const logDir = process.env.APP_LOG_PATH || path.join(__dirname, 'logs');
...
app.post('/api/pedidos', async (req, res) => {
    ...
    try {
        ...
        for (const quadroData of quadros) {
            ...
            const [moldurasSalvas, materiaisSalvos] = await Promise.all([
                Promise.all((quadroData.moldurasSelecionadas || []).map(async (nome) => {
                    ...
                    if (dbRow[0]) { await conn.execute(`INSERT INTO Quadro_Molduras (quadro_id, moldura_id) VALUES (?, ?)`, [quadroId, dbRow[0].id]); return { nome: dbRow[0].nome, codigo: dbRow[0].codigo }; } return null;
                })),
                Promise.all((quadroData.materiaisSelecionados || []).map(async (nome) => {
                    ...
                    if (dbRow[0]) { const esp = nome.toLowerCase() === 'paspatur' ? parseFloat(quadroData.espessuraPaspatur) || 0 : null; await conn.execute(`INSERT INTO Quadro_Materiais (quadro_id, material_id, espessura_paspatur_cm) VALUES (?, ?, ?)`, [quadroId, dbRow[0].id, esp]); return { nome: dbRow[0].nome, espessura_paspatur_cm: esp }; } return null;
                }))
            ]);
        ...
        const pdfOsBuffer = await generateOsPdf(dadosPedidoCompleto, quadrosAgrupados, globalLogoBase64, globalIconeWhatsappBase64);
        const pdfPedidoBuffer = await generateOrderPdf(dadosPedidoCompleto, quadrosAgrupados, globalLogoBase64, globalIconeWhatsappBase64);
        ...
        logger.error('--- ERRO DETALHADO AO SALVAR PEDIDO ---');
        logger.error(`Data e Hora: ${new Date().toISOString()}`);
        logger.error(`Mensagem do Erro: ${error.message}`);
        logger.error('Objeto do Pedido Recebido (parcial): ' + JSON.stringify({ nomeCliente, quadros: (quadros || []).length }));
        logger.error(`Stack Trace do Erro: ${error.stack}`);
        logger.error('------------------------------------');
        // --- FIM DO LOG ---
...
// ---- ROTA PARA DOWNLOAD DO PDF DO PEDIDO (OTIMIZADA) ----
app.get('/api/pedidos/:id/pdf', async (req, res) => {
    const pedidoId = req.params.id;
    try {
        const [rows] = await pool.execute('SELECT pdf_blob, pdf_filename FROM Pedidos WHERE id = ?', [pedidoId]);
        if (rows.length === 0 || !rows[0].pdf_blob) {
            return res.status(404).json({ message: 'PDF do pedido não encontrado.' });
        }
        const pdfBuffer = rows[0].pdf_blob;
        const filename = rows[0].pdf_filename || `pedido_${pedidoId}.pdf`;
        res.json({ success: true, pdfData: pdfBuffer.toString('base64'), filename });
    } catch (error) {
        console.error(`[ERRO_ROTA_PDF] Erro ao buscar PDF do Pedido ${pedidoId}:`, error);
        res.status(500).json({ message: `Erro ao buscar PDF do Pedido: ${error.message}` });
    }
});

// ---- ROTA PARA DOWNLOAD DO PDF DA OS (OTIMIZADA) ----
app.get('/api/pedidos/:id/os/pdf', async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0 || !rows[0].pdf_os_blob) {
            return res.status(404).json({ message: 'PDF da OS não encontrado.' });
        }
        const pdfBuffer = rows[0].pdf_os_blob;
        const filename = rows[0].pdf_os_filename || `os_${pedidoId}.pdf`;
        res.json({ success: true, pdfData: pdfBuffer.toString('base64'), filename });
    } catch (error) {
        console.error(`[ERRO_ROTA_OS_PDF] Erro ao buscar PDF da OS ${pedidoId}:`, error);
        res.status(500).json({ message: `Erro ao buscar PDF da OS: ${error.message}` });
    }
});
...
app.put('/api/pedidos/:id/status', async (req, res) => {
    ...
    const allowedStatuses = ['A Fazer', 'Já Feito', 'Entregue'];
...
app.listen(port, async () => {
    try {
        ...
        logger.info('[DB_CHECK] Conexão com o banco de dados bem-sucedida!');
        connection.release();

        const logoPath = path.join(__dirname, 'utils', 'logo_base64.txt');
        globalLogoBase64 = `data:image/png;base64,${await fs.promises.readFile(logoPath, 'utf8')}`;

        const iconePath = path.join(__dirname, 'utils', 'icone_whatsapp_base64.txt');
        globalIconeWhatsappBase64 = `data:image/png;base64,${await fs.promises.readFile(iconePath, 'utf8')}`;

        logger.info('[LOAD_ASSETS] Ativos Base64 carregados com sucesso.');
