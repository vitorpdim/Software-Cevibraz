...
async function generateOsPdf(pedidoData, quadrosAgrupadosData, logoBase64, iconeWhatsappBase64) {
    let browser;
    try {
        ...
        console.log(`[GERADOR_OS_PDF] PDF de OS gerado com sucesso. Tamanho do buffer: ${pdfBuffer.length}`);
        return pdfBuffer;

    } catch (error) {
        console.error('Erro ao gerar PDF da OS:', error);
        throw new Error(`Falha na geração de PDF da OS pelo Puppeteer: ${error.message || 'Erro desconhecido'}`);
    } finally {
        if (browser) {
            await browser.close();
        }
